...
import { deleteSessionById, getSessionById, updateSessionById } from '@/other/services/mongo/actions/session';
...
const StartPage = () => {
    ...
    const handleAppStartup = async () => {
        ...
        if (session.invalidAt.getTime() > (new Date).getTime()) {
        ...
        await updateSessionById(sessionId, {invalidAt: new Date((new Date()).getTime() + user.logoutAfter)})
